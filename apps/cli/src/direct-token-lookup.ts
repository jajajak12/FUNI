...
function initializeDirectLookupCandidates(repo:SqliteLedgerRepository,request:DirectLookupRequest,token:Address,selectedIds:string[],now:number){
 ...
 const cached=new Map(cachedV4PoolsForToken({repo,token,now}).candidates.map(item=>[item.poolId.toLowerCase(),item])),priorRows=repo.db.prepare(`SELECT c.* FROM direct_token_lookup_candidates c JOIN direct_token_lookup_requests r ON r.id=c.request_id AND r.revision=c.request_revision WHERE r.dedup_key=? AND r.revision<? AND c.state='ELIGIBLE' AND c.evidence_at_ms>=? ORDER BY r.revision DESC`).all(request.dedup_key,request.revision,now-DIRECT_LOOKUP_FRESH_MS) as DirectLookupCandidateLifecycleRow[],priorEligible=new Map<string,DirectLookupCandidateLifecycleRow>();
 for(const row of priorRows)if(!priorEligible.has(row.pool_id.toLowerCase()))priorEligible.set(row.pool_id.toLowerCase(),row);
 const insert=repo.db.prepare(`INSERT INTO direct_token_lookup_candidates(request_id,request_revision,pool_id,state,reason_code,attempt_count,requested_at_ms,completed_at_ms,evidence_at_ms,refresh_block,last_error) VALUES(?,?,?,?,?,0,?,?,?,?,NULL) ON CONFLICT(request_id,request_revision,pool_id) DO NOTHING`),run=repo.db.transaction(()=>{for(const id of selectedIds){const candidate=cached.get(id.toLowerCase()),registry=repo.v4RegistryPool(id),prior=priorEligible.get(id.toLowerCase()),registryRefreshMs=registry?.last_refreshed_at?Date.parse(String(registry.last_refreshed_at))||0:0,freshRegistryEligible=Boolean(candidate?.executionEligible),freshEligible=freshRegistryEligible||Boolean(prior),freshZero=candidate?.uiState==='SUPPORTED_NO_ACTIVE_LIQUIDITY',freshNotInitialized=Boolean(registryRefreshMs&&Math.max(0,now-registryRefreshMs)<=DIRECT_LOOKUP_FRESH_MS&&Number(registry?.initialized)!==1),state:DirectLookupCandidateState=freshEligible?'ELIGIBLE':freshZero?'NO_ACTIVE_LIQUIDITY':freshNotInitialized?'EVIDENCE_UNAVAILABLE':'REFRESH_REQUESTED',reason=freshEligible?'FRESH_POSITIVE_ACTIVE_LIQUIDITY':freshZero?'FRESH_ZERO_ACTIVE_LIQUIDITY':freshNotInitialized?'FRESH_STATEVIEW_NOT_INITIALIZED':'STATEVIEW_REFRESH_REQUESTED',freshEvidence=freshEligible||freshZero||freshNotInitialized,evidenceAt=freshRegistryEligible||freshZero||freshNotInitialized?now:prior?.evidence_at_ms??null,refreshBlock=freshRegistryEligible||freshZero||freshNotInitialized?candidate?.refreshBlock.toString()??registry?.refresh_block??null:prior?.refresh_block??null;insert.run(request.id,request.revision,id,state,reason,freshEvidence?null:now,freshEvidence?now:null,freshEvidence?evidenceAt:null,freshEvidence?refreshBlock:null);}});run();
}
...
type DirectLookupEvidenceCounts=ReturnType<typeof directLookupEvidenceCounts>;
function directLookupRegistryEvidence(repo:SqliteLedgerRepository,token:Address,now:number,rawCandidateCount:number):DirectLookupEvidenceCounts{const rows=directLookupStructuralCandidates(repo,token);let zeroLiquidityCandidateCount=0,notInitializedCandidateCount=0,freshPositive=0;for(const row of rows){const fresh=row.last_refreshed_at!==null&&rowRefreshMs(row)>=now-DIRECT_LOOKUP_FRESH_MS;if(!fresh)continue;if(Number(row.initialized)!==1)notInitializedCandidateCount++;else if(rowLiquidity(row)>0n)freshPositive++;else zeroLiquidityCandidateCount++;}const freshClassifiedCandidateCount=freshPositive+zeroLiquidityCandidateCount+notInitializedCandidateCount;return {structuralCandidateCount:rows.length,zeroLiquidityCandidateCount,notInitializedCandidateCount,unavailableCandidateCount:Math.max(0,rows.length-freshClassifiedCandidateCount),unsupportedCandidateCount:Math.max(0,rawCandidateCount-rows.length),freshClassifiedCandidateCount};}
function directLookupEvidenceForRequest(repo:SqliteLedgerRepository,request:DirectLookupRequest,now:number){
 const rows=directLookupCandidateLifecycle(repo,request.id,request.revision);if(rows.length){const counts=directLookupEvidenceCounts(rows);let plan:Record<string,unknown>={};try{plan=JSON.parse(String(request.rpc_attribution_json??'{}')) as Record<string,unknown>;}catch{}const structural=Math.max(counts.structuralCandidateCount,Number(plan.structuralCandidateCount??0)),unsupported=Math.max(counts.unsupportedCandidateCount,Number(plan.unsupportedCandidateCount??0)),unselected=Math.max(0,structural-counts.structuralCandidateCount);return {...counts,structuralCandidateCount:structural,unsupportedCandidateCount:unsupported,unavailableCandidateCount:counts.unavailableCandidateCount+unselected};}
...
export function completeDirectTokenLookup(repo:SqliteLedgerRepository,input:{
 ...
 providerResult:string;rpcAttribution:Partial<RpcAttribution>&Record<string,unknown>;reasonCode:string;nowMs?:number;evidence?:DirectLookupEvidenceCounts;
}){
 const now=input.nowMs??Date.now(),run=repo.db.transaction(()=>{
  ...
  const requestBefore=repo.db.prepare('SELECT * FROM direct_token_lookup_requests WHERE id=? AND revision=?').get(input.requestId,input.requestRevision) as DirectLookupRequest,evidence=input.evidence??directLookupEvidenceForRequest(repo,requestBefore,now),canonicalEligible=canonicalDirectLookupEligiblePoolIds(repo,requestBefore,now),eligiblePoolIds=[...new Map([...input.eligiblePoolIds,...canonicalEligible].map(id=>[id.toLowerCase(),id])).values()];let priorAttribution:Record<string,unknown>={};try{priorAttribution=JSON.parse(String(requestBefore.rpc_attribution_json??'{}')) as Record<string,unknown>;}catch{}
...
export async function executeDirectTokenLookup(input:{
 ...
 const yieldWrite=(operation:string)=>{const result=waitForEconomicForegroundDemandToClearSync({databasePath:input.repo.path,component:'robin-v4-direct-lookup-worker',operation,maxWaitMs:500,onTelemetry:event=>{try{process.stdout.write(JSON.stringify({event:'sqlite_write_window',...event,at:new Date().toISOString()})+'\n');}catch{}}});if(!result.cleared)throw new EconomicForegroundDemandActiveError(input.repo.path);return result;};
 const candidateStarted=now();input.onStage?.('candidate_discovery_start');const rawCandidateCount=directLookupRawCandidatePoolCount(input.repo,token),allocation=allocateDirectLookupCandidates(input.repo,token,candidateBudget,candidateStarted,false),candidateIds=allocation.candidateIds,fastWorkset={rpcIds:candidateIds.slice(0,6)},fairnessWorkset={rpcIds:candidateIds.slice(6,12)},rpcCandidateIds=candidateIds,initialEligibleIds=[...new Map([...freshExecutablePoolIds(input.repo,token,candidateStarted),...canonicalDirectLookupEligiblePoolIds(input.repo,input.request,candidateStarted)].map(id=>[id.toLowerCase(),id])).values()];yieldWrite('direct_lookup_candidate_persistence');withSqliteTransientRetrySync({operation:'direct_lookup_candidate_persistence',maxAttempts:5,maxTotalRetryMs:750,run:()=>input.repo.db.transaction(()=>{initializeDirectLookupCandidates(input.repo,input.request,token,rpcCandidateIds,candidateStarted);let prior:Record<string,unknown>={};try{prior=JSON.parse(String(input.request.rpc_attribution_json??'{}')) as Record<string,unknown>;}catch{}input.repo.db.prepare('UPDATE direct_token_lookup_requests SET candidate_pool_count=?,rpc_attribution_json=?,updated_at_ms=? WHERE id=? AND revision=?').run(rawCandidateCount,json({...prior,structuralCandidateCount:allocation.structuralCandidateCount,unsupportedCandidateCount:Math.max(0,rawCandidateCount-allocation.structuralCandidateCount),selectedCandidateCount:rpcCandidateIds.length}),candidateStarted,input.request.id,input.request.revision);for(const id of rpcCandidateIds)input.repo.enqueueV4StateRefresh(id,90,'recent-token-lookup',candidateStarted);})()});input.request=input.repo.db.prepare('SELECT * FROM direct_token_lookup_requests WHERE id=? AND revision=?').get(input.request.id,input.request.revision) as DirectLookupRequest;input.onStage?.('candidate_discovery_end',{rawCandidateCount,boundedCandidateCount:candidateIds.length,rpcCandidateCount:rpcCandidateIds.length,durableBlockedCount:rawCandidateCount-rpcCandidateIds.length,fastCandidateCount:allocation.fastIds.length,fairnessCandidateCount:allocation.fairnessIds.length,fastFairnessOverlap:allocation.fastFairnessOverlap,fairnessCursorBefore:allocation.fairnessCursorBefore,fairnessCursorAfter:allocation.fairnessCursorAfter,freshExecutableCacheBefore:initialEligibleIds.length,elapsedMs:now()-candidateStarted});const attributed=attributedRpc(input.rpc,provider,input.ethCallBudget??candidateBudget*2);
