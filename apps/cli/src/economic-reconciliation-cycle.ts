...
import { reconcileExternalV4MutationCycle } from './external-v4-mutation-reconciliation.js';
...
export async function runEconomicReconciliationCycle(input:{repo:SqliteLedgerRepository;rpc:FallbackRpc;logsRpc?:FallbackRpc;owner?:string;nowMs?:number;limit?:number}){
 ...
 const externalMutations=await reconcileExternalV4MutationCycle({repo:input.repo,rpc:input.rpc,logsRpc:input.logsRpc,limit:2,nowMs:Date.now()});
 return {discovery,leased:leased.length,results,externalMutations,signingAttempts:0 as const,broadcasts:0 as const,mainnetTransactionsSent:0 as const};
}
