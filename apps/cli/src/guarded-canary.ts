...
async function assertFreshV3EntryPrice(input:GuardedCanaryInput){
 if(input.entryPricePreflight){const result=await input.entryPricePreflight();if(result.status!=='PASS')throw new Error(result.blocker??'LP_ENTRY_PRICE_GUARD_BLOCKED');return;}
 const raw=await inspectV3Pool(input.rpc,input.pool);if(raw.status==='unavailable')throw new Error('LP_ENTRY_POOL_PRICE_UNAVAILABLE');const shown=await presentPool(input.rpc,raw.value);if(shown.status==='unavailable')throw new Error('LP_ENTRY_POOL_PRICE_UNAVAILABLE');
 const fundingUsd=input.funding.address.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?1:input.funding.address.toLowerCase()===robinhoodMainnet.assets.WETH.toLowerCase()?await trustedWethUsdReference(input.rpc).then(value=>{if(value.status==='unavailable')throw new Error(value.reason);return value.value;}):NaN;
 if(!Number.isFinite(fundingUsd)||fundingUsd<=0)throw new Error('LP_ENTRY_FUNDING_PRICE_UNAVAILABLE');
 const poolPrice=orientPoolPriceFundingPerTarget({priceToken1PerToken0:shown.value.priceToken1PerToken0,token0:shown.value.token0.address,token1:shown.value.token1.address,target:input.target.address,funding:input.funding.address}),result=await freshLpEntryPriceGuard({target:input.target.address,poolPriceFundingPerTarget:poolPrice,fundingUsd});if(result.status!=='PASS')throw new Error(result.blocker??'LP_ENTRY_PRICE_GUARD_BLOCKED');
}
...
export async function executeGuardedSingleSidedCanary(input: GuardedCanaryInput) {
  ...
  if (recoveryState === 'PREVIEWED') {
    try{await assertFreshV3EntryPrice(input);}catch(error){return {ok:false as const,status:'EXECUTION_BLOCKED' as const,reason:error instanceof Error?error.message:String(error)};}
  ...
  try {
    return await rpc.withClient(async client => {
      ...
      if (balance < input.fundingAmount) throw new Error('funding balance changed');
        ...
        await assertFreshV3EntryPrice(input);
