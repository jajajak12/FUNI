...
import { enqueuePortfolioRefresh, markOperationalPositionOpenConfirming } from './active-position-reconciliation.js';
...
export type V4OperationalOpenPreflightInput={
  ...
  entryPricePreflight?:()=>Promise<{status:'PASS'|'BLOCK';blocker:string|null}>;
  intentId?:string;
  gasPriceWei?:bigint;
...
async function assertFreshV4EntryPrice(input:V4OperationalOpenPreflightInput){
 const injected=input.entryPricePreflight;if(injected){const result=await injected();if(result.status!=='PASS')throw new Error(result.blocker??'LP_ENTRY_PRICE_GUARD_BLOCKED');return;}
 const key:V4PoolKey={currency0:getAddress(input.selection.key.currency0),currency1:getAddress(input.selection.key.currency1),fee:input.selection.key.fee,tickSpacing:input.selection.key.tickSpacing,hooks:getAddress(input.selection.key.hooks)},state=await inspectV4Pool(input.rpc,key);if(state.status==='unavailable')throw new Error('LP_ENTRY_POOL_PRICE_UNAVAILABLE');
 const d0=Number(input.selection.targetIndex===0?input.selection.targetDecimals:input.selection.fundingDecimals),d1=Number(input.selection.targetIndex===1?input.selection.targetDecimals:input.selection.fundingDecimals);if(!Number.isInteger(d0)||!Number.isInteger(d1)||d0<0||d1<0)throw new Error('LP_ENTRY_TOKEN_DECIMALS_INVALID');const poolPrice=orientPoolPriceFundingPerTarget({priceToken1PerToken0:priceFromSqrtX96(state.value.sqrtPriceX96,d0,d1),token0:key.currency0,token1:key.currency1,target:input.selection.target,funding:input.selection.funding}),result=await freshLpEntryPriceGuard({target:getAddress(input.selection.target),poolPriceFundingPerTarget:poolPrice,fundingUsd:input.fundingUsd});
 if(result.status!=='PASS')throw new Error(result.blocker??'LP_ENTRY_PRICE_GUARD_BLOCKED');
}
...
export async function v4OperationalOpenPreflight(input:V4OperationalOpenPreflightInput):Promise<V4OperationalOpenPreflightResult>{
  ...
  return {
    ...
    positionUsd,approvalUsd,pool:poolSnapshot,fundingBalance,hasOpenIntent:pending,
...
function operationalSelection(row:Record<string,unknown>):GenericV4OpenSelection{const payload=JSON.parse(String(row.payload_json)),raw=payload.genericSelection;if(!raw)throw new Error('V4_OPERATIONAL_SELECTION_MISSING');return {...raw,key:{...raw.key,currency0:getAddress(raw.key.currency0),currency1:getAddress(raw.key.currency1),hooks:getAddress(raw.key.hooks)},target:getAddress(raw.target),funding:getAddress(raw.funding),amount:BigInt(String(row.amount_raw))};}
function operationalReceipt(repo:SqliteLedgerRepository,intentId:string,hash:string){const row=repo.db.prepare("SELECT receipt_json FROM v4_operational_open_receipts WHERE intent_id=? AND phase='V4_MINT' AND lower(tx_hash)=lower(?)").get(intentId,hash) as {receipt_json:string}|undefined;if(!row)throw new Error('V4_OPERATIONAL_MINT_RECEIPT_MISSING');return JSON.parse(row.receipt_json) as TransactionReceipt;}
function operationalPreparedMint(repo:SqliteLedgerRepository,intentId:string){const row=repo.db.prepare("SELECT details_json FROM v4_live_transitions WHERE intent_id=? AND state='V4_MINT_PREPARED' ORDER BY ordinal DESC LIMIT 1").get(intentId) as {details_json:string}|undefined;if(!row)throw new Error('V4_OPERATIONAL_MINT_PREPARED_MISSING');const details=JSON.parse(row.details_json),data=details.request?.data;if(typeof data!=='string')throw new Error('V4_OPERATIONAL_MINT_PREPARED_MALFORMED');const decoded=decodeV4BatchMint(data as Hex);if(decoded.legs.length!==1)throw new Error('V4_OPERATIONAL_MINT_PLAN_AMBIGUOUS');return decoded.legs[0]!;}
export function persistConfirmedV4OperationalOpenProjection(repo:SqliteLedgerRepository,intentId:string){const row=repo.v4LiveOpenIntent(intentId) as Record<string,unknown>|undefined;if(!row||!row.mint_hash)throw new Error('V4_OPERATIONAL_MINT_HASH_MISSING');const receipt=operationalReceipt(repo,intentId,String(row.mint_hash));if(receipt.status!=='success'||String(receipt.transactionHash).toLowerCase()!==String(row.mint_hash).toLowerCase())throw new Error('V4_OPERATIONAL_MINT_RECEIPT_INVALID');const selection=operationalSelection(row),plan=operationalPreparedMint(repo,intentId),tokenId=parseV4MintTokenId(receipt.logs, getAddress(String(row.owner))),spent=fundingSpent(receipt.logs,getAddress(String(row.owner)),getAddress(selection.funding));if(spent<=0n||spent>selection.amount||plan.liquidity<=0n)throw new Error('V4_MINT_ACCOUNTING_INVARIANT_FAILED');repo.db.transaction(()=>{repo.transitionV4LiveOpenIntent(intentId,'MINT_CONFIRMED',{tokenId:tokenId.toString(),details:{fundingSpent:spent,liquidity:plan.liquidity,reconciliationPending:true}});persistOpenedPosition({repo,wallet:getAddress(String(row.owner)),selection,tokenId,liquidity:plan.liquidity,tickLower:plan.tickLower,tickUpper:plan.tickUpper,fundingSpent:spent,mintHash:receipt.transactionHash,blockNumber:receipt.blockNumber,intentId});})();enqueuePortfolioRefresh(repo,'V4_OPERATIONAL_OPEN_RECEIPT_CONFIRMED');return {tokenId,fundingSpent:spent,mintHash:receipt.transactionHash};}
...
export async function executeV4OperationalOpen(input:V4OperationalOpenPreflightInput&{intentId:string;idempotencyKey:string;walletClient:WalletClient}):Promise<V4OperationalExecutionResult>{
  ...
  try{
    ...
    if(String(current.state)==='PREVIEWED')await assertFreshV4EntryPrice(input);
    ...
    const acquireNonce=async(nonce:bigint)=>{
      const deadline=Date.now()+30_000;
      do{
        if(input.repo.acquireNonceMutex(input.wallet,nonce,660))return;
        await delay(100);
      }while(Date.now()<deadline);
      throw new Error('V4_NONCE_MUTEX_HELD');
    };
    ...
    const resendPersisted=async(phase:string,hash:Hash)=>{
      ...
      let confirmed:TransactionReceipt|undefined,pending=false;
      try{
        const before=await exactHashEvidence(input.rpc,input.wallet,hash,nonce);if(before.kind==='RECEIPT'){confirmed=before.receipt;}else if(before.kind==='PENDING'){input.repo.transitionV4LiveOpenIntent(input.intentId,`${phase}_SUBMITTED`);pending=true;}else if(before.kind==='INCONCLUSIVE')throw new Error(`V4_BROADCAST_AMBIGUOUS:${before.reason}`);else if(before.latestNonce!==nonce||before.pendingNonce!==nonce)throw new Error('V4_BROADCAST_EXACT_HASH_ABSENT_NONCE_CONSUMED_OR_REPLACED');
        if(!confirmed&&!pending){
         ...
         try{await broadcastSignedTransaction({walletClient:endpoint.walletClient,serializedTransaction:serialized,expectedHash:hash,expectedSender:input.wallet,expectedChainId:input.rpc.config.chainId,expectedNonce:nonce,providerIndex:endpoint.providerIndex,providerName:`${endpoint.providerType}-${endpoint.providerIndex}`,onEvidence:evidence=>input.log?.('transaction_broadcast_transport',evidence)});sent++;input.repo.transitionV4LiveOpenIntent(input.intentId,`${phase}_SUBMITTED`);pending=true;await input.notify?.(`${phase}_SUBMITTED`,{hash,identicalResend:true,providerIndex:endpoint.providerIndex});break;}
         catch{
          const after=await exactHashEvidence(input.rpc,input.wallet,hash,nonce);if(after.kind==='RECEIPT'){confirmed=after.receipt;break;}if(after.kind==='PENDING'){input.repo.transitionV4LiveOpenIntent(input.intentId,`${phase}_SUBMITTED`);pending=true;break;}const reason=after.kind==='INCONCLUSIVE'?`V4_BROADCAST_AMBIGUOUS:${after.reason}`:after.latestNonce===nonce&&after.pendingNonce===nonce?'V4_BROADCAST_PROVEN_ABSENT_AFTER_IDENTICAL_RESEND':'V4_BROADCAST_EXACT_HASH_ABSENT_NONCE_CONSUMED_OR_REPLACED';if(after.kind==='INCONCLUSIVE'||reason!=='V4_BROADCAST_PROVEN_ABSENT_AFTER_IDENTICAL_RESEND'||endpointOrdinal===endpoints.length-1){input.repo.transitionV4LiveOpenIntent(input.intentId,after.kind==='INCONCLUSIVE'?`${phase}_BROADCAST_AMBIGUOUS`:'FAILED',{failureReason:reason,details:{phase,exactHash:hash,nonce,providerIndex:endpoint.providerIndex,providerType:endpoint.providerType,evidence:after}});throw new Error(reason);}
         }
        }
        }
      }finally{input.repo.releaseNonceMutex(input.wallet);}
      if(confirmed)return reconcile(phase,hash,confirmed);
      if(pending)return reconcile(phase,hash);
    ...
    let projection:{tokenId:bigint;fundingSpent:bigint;mintHash:Hash},durableHandoff=true;try{projection=persistConfirmedV4OperationalOpenProjection(input.repo,input.intentId);}catch(error){if(!sqliteTransientCode(error))throw error;projection={tokenId:parseV4MintTokenId(mintReceipt.logs,input.wallet),fundingSpent:fundingSpent(mintReceipt.logs,input.wallet,getAddress(selection.funding)),mintHash};durableHandoff=false;}try{ensureEconomicReconciliationWork(input.repo,{chainId:4663,workflowKind:'V4_OPERATIONAL_OPEN',workflowIdentity:input.intentId,semanticStage:'V4_MINT',transactionHash:mintHash,sourceTable:'v4_live_open_intents',sourceIdentity:input.intentId,priority:1_000});}catch(error){if(!sqliteTransientCode(error))throw error;durableHandoff=false;}
    return {status:'OPEN_RECONCILIATION_PENDING',intentId:input.intentId,mintHash,tokenId:projection.tokenId,fundingSpent:projection.fundingSpent,durableHandoff,mainnetTransactionsSent:sent};
  }catch(error){
    const reason=error instanceof Error?error.message:String(error);
    const durable=input.repo.v4LiveOpenIntent(input.intentId),state=String(durable?.state??'');
    const receiptBacked=Boolean(durable?.mint_hash&&input.repo.db.prepare("SELECT 1 FROM v4_operational_open_receipts WHERE intent_id=? AND phase='V4_MINT' AND lower(tx_hash)=lower(?)").get(input.intentId,String(durable.mint_hash)));if(!receiptBacked&&!/_PREPARED$|_SUBMITTED$|_BROADCAST_AMBIGUOUS$/.test(state))input.repo.transitionV4LiveOpenIntent(input.intentId,'FAILED',{failureReason:reason});
    throw error;
  }finally{
