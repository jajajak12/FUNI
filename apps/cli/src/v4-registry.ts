...
import { inspectErc20, priceFromSqrtX96, robinhoodMainnet, type FallbackRpc } from '@robin/core';
...
import {
  ...
  inspectV4Pool, poolId, readV4InitializeEvents, v4ExecutionBlockers, V4_ROBINHOOD_DEPLOYMENTS, v4StateViewAbi, type V4InitializeEvent, type V4PoolKey,
} from '@robin/v4';
...
export type V4Candidate={
 ...
 uiState?:'CHECKING'|'TEMPORARILY_UNAVAILABLE'|'EXECUTABLE'|'SUPPORTED_NO_ACTIVE_LIQUIDITY'|`UNSUPPORTED:${string}`;
...
export async function syncV4PoolRegistry(input:{repo:SqliteLedgerRepository;rpc:FallbackRpc;toBlock?:bigint;confirmations?:number;reader?:V4RegistryReader;onEvent?:(name:string,data:Record<string,unknown>)=>void}){
 ...
 try{
  while(from<=target){
   ...
   const persistWindow=input.repo.db.transaction(()=>{for(const event of events){const existed=!!input.repo.v4RegistryPool(event.id);input.repo.upsertV4RegistryPool(eventRecord(event));existed?updated++:inserted++;}});persistWindow();
...
export async function refreshV4RegistryPoolBatch(input:{repo:SqliteLedgerRepository;rpc:FallbackRpc;poolIds:string[]}){
 ...
 if(!rows.length)return {refreshed:[] as string[],failed:[] as string[],multicallPoolCount:0};
 const response=await input.rpc.withClient(async client=>({block:await client.getBlockNumber(),results:await client.multicall({multicallAddress:'0xca11bde05977b3631167028862be2a173976ca11',allowFailure:true,contracts:keys.flatMap(key=>[{address:V4_ROBINHOOD_DEPLOYMENTS.stateView,abi:v4StateViewAbi,functionName:'getSlot0',args:[poolId(key)]},{address:V4_ROBINHOOD_DEPLOYMENTS.stateView,abi:v4StateViewAbi,functionName:'getLiquidity',args:[poolId(key)]}])})}));
 const refreshed:string[]=[],failed:string[]=[];
 for(let i=0;i<ids.length;i++){const slot=response.results[i*2],liquidity=response.results[i*2+1],row=rows[i]!,key=keys[i]!;if(slot?.status!=='success'||liquidity?.status!=='success'){failed.push(ids[i]!);continue;}const value=slot.result as readonly [bigint,bigint,bigint,bigint],state={key,id:poolId(key),sqrtPriceX96:value[0],tick:Number(value[1]),protocolFee:Number(value[2]),lpFee:Number(value[3]),liquidity:liquidity.result as bigint,initialized:value[0]!==0n,blockNumber:response.block};const blockers=v4ExecutionBlockers(state);input.repo.refreshV4RegistryPool({poolId:ids[i]!,sqrtPriceX96:state.sqrtPriceX96,tick:state.tick,liquidity:state.liquidity,protocolFee:state.protocolFee,lpFeePips:state.lpFee,initialized:state.initialized,refreshBlock:state.blockNumber,validationStatus:blockers.length?'BLOCKED':'ELIGIBLE',blockers});refreshed.push(ids[i]!);}
 return {refreshed,failed,multicallPoolCount:ids.length};
}
...
/** Active-liquidity eligibility with a deterministic PoolId tie-break. */
export function rankV4Candidates(candidates:V4Candidate[]){
 return [...candidates].sort((a,b)=>Number(b.executionEligible)-Number(a.executionEligible)||a.poolId.localeCompare(b.poolId));
}
...
export function cachedV4PoolsForToken(input:{repo:SqliteLedgerRepository;token:Address;fundingAssets?:Address[];now?:number}){
 ...
 const candidates:V4Candidate[]=rows.map(row=>{
  ...
  const unsupported=[...new Set([...persisted,...fee.blockers,...hooks.blockers].filter(x=>x!=='ZERO_ACTIVE_LIQUIDITY'))],initialized=Number(row.initialized)===1,stateFresh=cacheAgeMs!==null&&cacheAgeMs<=120_000;
