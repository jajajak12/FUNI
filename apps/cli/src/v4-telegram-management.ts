...
export async function confirmV4TelegramManagement(input:Omit<V4TelegramActionInput,'messageId'>&{intentId:string}){
  ...
  const result=await executeV4Lifecycle({...input,idempotencyKey:String(intent.idempotency_key)});
  if(input.action!=='full_close'||!result.ok)return result;
  ...
  return burn.ok?{...result,burn}:{...burn,fullClose:result};
}
