import { validateV4DownsideRange, type V4DownsideRangeRequest } from '@robin/v4';
import type { SqliteLedgerRepository } from '@robin/ledger';

export const V4_RANGE_PRESETS=[10,30,50,60] as const;
export function v4RangeButtons(selectionId:string){return [...V4_RANGE_PRESETS.map(value=>[{label:`USDG-only current → -${value}%`,data:`v4-range:${selectionId}:${value}`}]),[{label:'USDG-only Custom range',data:`v4-range:${selectionId}:custom`}]];}
