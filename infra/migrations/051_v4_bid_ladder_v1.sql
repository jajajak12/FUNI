...
CREATE TABLE v4_bid_ladder_legs (
  ...
  upper_drop_bps INTEGER NOT NULL CHECK(upper_drop_bps >= 0),
  lower_drop_bps INTEGER NOT NULL CHECK(lower_drop_bps > upper_drop_bps),
  ...
  CHECK(
    (leg_index = 0 AND upper_drop_bps = 100 AND lower_drop_bps = 500 AND capital_weight_bps = 800) OR
    (leg_index = 1 AND upper_drop_bps = 500 AND lower_drop_bps = 1200 AND capital_weight_bps = 1200) OR
    (leg_index = 2 AND upper_drop_bps = 1200 AND lower_drop_bps = 2200 AND capital_weight_bps = 1800) OR
    (leg_index = 3 AND upper_drop_bps = 2200 AND lower_drop_bps = 3500 AND capital_weight_bps = 2500) OR
    (leg_index = 4 AND upper_drop_bps = 3500 AND lower_drop_bps = 5000 AND capital_weight_bps = 3700)
  )
);
