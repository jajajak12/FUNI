import { decodeAbiParameters, decodeEventLog, decodeFunctionData, encodeAbiParameters, encodeFunctionData, getAddress, keccak256, parseAbiItem, toHex, zeroAddress, type Address, type Hex } from 'viem';
import { RH_MAINNET, evaluateDeploymentVerification, inspectErc20, isRetryableRpcFailure, priceFromSqrtX96, robinhoodMainnet, sanitizeRpcError, type Availability, type DeploymentVerificationResult, type ExplorerStatus, type FallbackRpc } from '@robin/core';
...
export function v4ExecutionBlockers(state:V4PoolState,extremeFeePips=50_000):string[]{
...
export function buildGenericV4SingleSidedDownsidePlan(input:{pool:V4PoolState;target:Address;funding:Address;fundingAmount:bigint;owner:Address;deadline:bigint;range:V4DownsideRangeRequest}):V4MintPlan&{targetIndex:0|1;fundingIndex:0|1;currentTick:number;sqrtPriceX96:bigint;amount0Expected:bigint;amount1Expected:bigint;requestedRange:V4DownsideRangeRequest;effectiveRange:V4DownsideRangeRequest}{
 const {pool,target,funding,fundingAmount,owner,deadline,range}=input;validateV4DownsideRange(range);
 ...
 if(targetIndex===0){tickUpper=Math.floor((pool.tick+offset(range.upperDropPct))/spacing)*spacing;tickLower=Math.floor((pool.tick+offset(range.lowerDropPct))/spacing)*spacing;if(pool.tick<tickUpper)throw new Error('V4_NOT_STRICT_FUNDING_ONLY');}
 else {tickLower=Math.ceil((pool.tick-offset(range.upperDropPct))/spacing)*spacing;tickUpper=Math.ceil((pool.tick-offset(range.lowerDropPct))/spacing)*spacing;if(pool.tick>tickLower)throw new Error('V4_NOT_STRICT_FUNDING_ONLY');}
 ...
 return {...buildV4Mint({key:pool.key,tickLower,tickUpper,liquidity,amount0Max:fundingIndex===0?fundingAmount:0n,amount1Max:fundingIndex===1?fundingAmount:0n,owner,hookData:'0x',deadline,fundingIndex:fundingIndex as 0|1}),targetIndex:targetIndex as 0|1,fundingIndex:fundingIndex as 0|1,currentTick:pool.tick,sqrtPriceX96:pool.sqrtPriceX96,amount0Expected,amount1Expected,requestedRange:{...range},effectiveRange};
}
...
export async function inspectV4PositionState(rpc:FallbackRpc,tokenId:bigint){
 const position=await inspectV4Position(rpc,tokenId),pool=await inspectV4Pool(rpc,position.key);
 ...
 const amounts=amountsForLiquidity(pool.value.sqrtPriceX96,position.tickLower,position.tickUpper,position.liquidity);
 const rangeState=classifyV4RangeState(pool.value.tick,position.tickLower,position.tickUpper);
 const price1Per0=priceFromSqrtX96(pool.value.sqrtPriceX96,token0.value.decimals,token1.value.decimals);
 const usd0=position.key.currency0.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?1:position.key.currency1.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?price1Per0:undefined;
 const usd1=position.key.currency1.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?1:position.key.currency0.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?1/price1Per0:undefined;
 const currentUsdValue=usd0!==undefined&&usd1!==undefined?Number(amounts.token0)/10**token0.value.decimals*usd0+Number(amounts.token1)/10**token1.value.decimals*usd1:null;
 ...
 const id=poolId(position.key),[stored,growth]=await rpc.withClient(client=>Promise.all([
  client.readContract({address:V4_ROBINHOOD_DEPLOYMENTS.stateView,abi:stateViewAbi,functionName:'getPositionInfo',args:[id,V4_ROBINHOOD_DEPLOYMENTS.positionManager,position.tickLower,position.tickUpper,toHex(tokenId,{size:32})]}),
  client.readContract({address:V4_ROBINHOOD_DEPLOYMENTS.stateView,abi:stateViewAbi,functionName:'getFeeGrowthInside',args:[id,position.tickLower,position.tickUpper]}),
 ]),{stage:'v4_fee_inspection',method:'StateView.getPositionInfo+getFeeGrowthInside'});
 ...
 return {...position,pool:pool.value,token0:token0.value,token1:token1.value,currentAmounts:amounts,rangeState,price1Per0,currentUsdValue,valuation,claimableFees};
}
