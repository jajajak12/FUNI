...
describe('durable direct token lookup',()=>{
 ...
 it('keeps cached lookup and Telegram handlers free of hydration polling and fan-out side effects',()=>{
  ...
  expect(cached).not.toMatch(/\bnoteTokenRequest\b|\benqueueV4StateRefresh\b/);expect(telegram).not.toMatch(/\bscheduleHydrationEdit\b|\bnoChangeRenderCount\b/);expect(telegram).toMatch(/\bpollingIterations\s*:\s*0\b/);
 });
 ...
 it('uses WAL plus busy timeout and releases write phases before network work',async()=>{
  const f=fixture(0);try{
   ...
   const holder=spawn(process.execPath,['-e',`const DB=require('better-sqlite3'),db=new DB(${JSON.stringify(f.repo.path)});db.exec('BEGIN IMMEDIATE');process.stdout.write('locked');process.stdin.once('data',()=>{db.exec('COMMIT');db.close();process.stdout.write('released')})`],{cwd:process.cwd(),stdio:['pipe','pipe','inherit']}),writer=spawn(process.execPath,['-e',`const DB=require('better-sqlite3');process.stdin.once('data',()=>{const db=new DB(${JSON.stringify(f.repo.path)});db.pragma('busy_timeout = 10000');process.stdout.write('attempting');db.prepare("INSERT INTO latency_telemetry(metric,duration_ms,fallback_used,context_json,created_at_ms) VALUES('busy-test',0,0,'{}',1)").run();db.close();process.stdout.write('completed')})`],{cwd:process.cwd(),stdio:['pipe','pipe','inherit']});
   const waitFor=(child:ReturnType<typeof spawn>,word:string)=>new Promise<void>((resolve,reject)=>{child.stdout.on('data',chunk=>{if(String(chunk).includes(word))resolve();});child.once('error',reject);});const holderClosed=new Promise(resolve=>holder.once('close',resolve)),writerClosed=new Promise(resolve=>writer.once('close',resolve));await waitFor(holder,'locked');let writerCompleted=false;writer.stdout.on('data',chunk=>{if(String(chunk).includes('completed'))writerCompleted=true;});const attempting=waitFor(writer,'attempting'),holderReleased=waitFor(holder,'released'),writerDone=waitFor(writer,'completed');writer.stdin.end('attempt');await attempting;await new Promise<void>(resolve=>setImmediate(resolve));expect(writerCompleted).toBe(false);holder.stdin.end('release');await holderReleased;await writerDone;expect(writerCompleted).toBe(true);await Promise.all([holderClosed,writerClosed]);
