...
import { backupSqlite, migrateSqlite, productionDatabasePaths, restoreSqliteBackup, SqliteLedgerRepository, sqliteStatus } from '@robin/ledger';
...
describe('durable production ledger runtime',()=>{
 it('migrates repeatably, uses restrictive paths, and restores an atomic backup',async()=>{
  const root=mkdtempSync(join(tmpdir(),'robin-durable-')),paths=productionDatabasePaths({dataDir:root}),migrations=join(process.cwd(),'infra/migrations');
  try{
 ...
 it('makes confirmations single-use, rejects stale requests, and keeps reconciliation idempotent',()=>{
  const root=mkdtempSync(join(tmpdir(),'robin-confirm-')),path=join(root,'ledger.sqlite'),migrations=join(process.cwd(),'infra/migrations');
  try{
