...
describe('durable rebalance transaction journal',()=>{
 ...
 it('blocks an estimated gas breach before signing or broadcast',async()=>{const f=fixture();let signs=0,sends=0;try{await expect(executeRebalanceTransaction(txInput(f,'OPEN_MINT',rpc([client({estimateGas:async()=>1n,getGasPrice:async()=>1_000_000_000_000_000_000n})]),{account:{signTransaction:async()=>{signs++;return '0x1234';}},signTransaction:async()=>{signs++;return '0x1234';},sendRawTransaction:async()=>{sends++;return keccak256('0x1234');}}))).rejects.toThrow('TX_GAS_CAP_EXCEEDED');expect({signs,sends}).toEqual({signs:0,sends:0});expect(rows(f,'OPEN_MINT')).toHaveLength(0);}finally{f.close();}});

 it('persists an exact successful receipt before recording an actual-gas breach and pauses later stages',async()=>{const f=fixture(),hash=keccak256('0x313233'),stage=insertAttempt(f,{hash,nonce:145,status:'SUBMITTED'}),expensive={...successfulReceipt(hash),gasUsed:1n,effectiveGasPrice:1_000_000_000_000_000_000n};let sends=0;try{const result=await executeRebalanceTransaction(txInput(f,stage,rpc([client({getTransactionReceipt:async()=>expensive})]),{sendRawTransaction:async()=>{sends++;return hash;}}));expect(result.actualGasPolicyBreach).toContain('ACTUAL_TX_GAS_CAP_EXCEEDED');expect(rows(f,stage)[0]).toMatchObject({status:'CONFIRMED',tx_hash:hash,receipt_json:expect.any(String),actual_gas_raw:'1',actual_gas_usd:1,failure_reason:expect.stringContaining('ACTUAL_GAS_POLICY_BREACH')});const recovered=await executeRebalanceTransaction(txInput(f,stage,{config:{chainId:4663},withClient:async()=>{throw new Error('RPC_FORBIDDEN');}},{sendRawTransaction:async()=>{sends++;return hash;}}));expect(recovered).toMatchObject({hash,recovered:true,actualGasPolicyBreach:expect.stringContaining('ACTUAL_TX_GAS_CAP_EXCEEDED')});await expect(executeRebalanceTransaction(txInput(f,'NEXT_STAGE',rpc([client()]),{sendRawTransaction:async()=>{sends++;return hash;}}))).rejects.toThrow(`REBALANCE_GAS_POLICY_PAUSED_AFTER_CONFIRMED_STAGE:${stage}`);expect(sends).toBe(0);expect(rows(f,stage)).toHaveLength(1);}finally{f.close();}});

 it('confirms raw gas and pauses recovery when USD valuation is temporarily unavailable',async()=>{const f=fixture(),hash=keccak256('0x323334'),stage=insertAttempt(f,{hash,nonce:145,status:'SUBMITTED'}),input=txInput(f,stage,rpc([client({getTransactionReceipt:async()=>successfulReceipt(hash)})]),{});input.limits={...limits,nativeUsd:Number.NaN};try{const result=await executeRebalanceTransaction(input);expect(result.actualGasPolicyBreach).toContain('ACTUAL_GAS_USD_VALUATION_UNAVAILABLE');expect(rows(f,stage)[0]).toMatchObject({status:'CONFIRMED',actual_gas_raw:'90000',actual_gas_usd:null,receipt_json:expect.any(String),failure_reason:'ACTUAL_GAS_POLICY_BREACH:ACTUAL_GAS_USD_VALUATION_UNAVAILABLE'});}finally{f.close();}});
});
