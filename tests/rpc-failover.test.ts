...
import { discoverV3Pools, FallbackRpc, orderedRpcUrls, robinhoodMainnet, sanitizeRpcError, type RpcFailoverEvent } from '@robin/core';
...
describe('bounded RPC failover',()=>{
 ...
 it('does not rotate a semantic validation failure',async()=>{
  let fallbackCalls=0;
  const rpc=new FallbackRpc({...robinhoodMainnet,rpcUrls:['https://one.example','https://two.example']},undefined,{clients:[client(async()=>{throw new Error('REBALANCE_POSITION_OR_POOL_INELIGIBLE');}),client(async()=>{fallbackCalls++;return 'unexpected';})]});
  await expect(rpc.withClient(c=>(c as any).read())).rejects.toThrow('REBALANCE_POSITION_OR_POOL_INELIGIBLE');
  expect(fallbackCalls).toBe(0);
 });
