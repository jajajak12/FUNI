...
import { jsonStringify, migrateSqlite, SqliteLedgerRepository } from '@robin/ledger';
import { safeTelegramError, safeTelegramOperation } from '../apps/telegram-lp-bot/src/telegram-error.js';

function fixture(){const dir=mkdtempSync(join(tmpdir(),'robin-telegram-bigint-')),path=join(dir,'db.sqlite');migrateSqlite(path,'infra/migrations');const repo=new SqliteLedgerRepository(path);return {repo,close(){repo.close();rmSync(dir,{recursive:true,force:true});}};}
describe('Telegram durable BigInt persistence',()=>{
 ...
 it('never includes a synthetic context secret in scalar error metadata',()=>{const secret='123456:synthetic_secret_value_that_must_not_appear';const details=safeTelegramError(new Error(`failure ${secret}`));const rendered=JSON.stringify({operation:safeTelegramOperation({callbackQuery:{data:'v4-range:session:10'},api:{token:secret}}),...details});expect(rendered).not.toContain(secret);expect(rendered).toContain('v4-range');});
});
