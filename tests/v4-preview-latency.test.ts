...
import { poolId,V4_ROBINHOOD_DEPLOYMENTS,type V4PoolKey } from '@funi/v4';
...
describe('operational amount preview latency',()=>{
 it('keeps cold and warm mocked previews bounded and accounts all safety stages',async()=>{
  ...
  const f=fixture(40);try{
   ...
   console.log(JSON.stringify({event:'public_amount_preview_performance',progressPaintMs:0,coldAuthoritativeFinalMs:coldMs,warmAuthoritativeFinalMs:warmMs,fixtureRpcLatencyMs:40}));
