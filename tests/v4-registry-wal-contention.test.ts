...
describe('V4 registry WAL ownership',()=>{
 ...
 it('does not wrap a registry event window in one WAL-writer transaction',()=>{const source=readFileSync('apps/cli/src/v4-registry.ts','utf8'),sync=source.slice(source.indexOf('export async function syncV4PoolRegistry'),source.indexOf('/** Explicit one-time bootstrap'));expect(sync).not.toContain('persistWindow');expect(sync).not.toContain('repo.db.transaction');expect(sync.indexOf('for(const event of events)')).toBeLessThan(sync.indexOf('advanceV4RegistryCursor'));});
});
