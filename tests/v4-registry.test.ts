...
import { classifyV4Hooks, decodeV4Fee, poolId, V4_DYNAMIC_FEE_FLAG, type V4InitializeEvent, type V4PoolKey } from '@funi/v4';
...
import { trustedV4PoolUsdMetric } from '../apps/cli/src/v4-liquidity-display.js';
...
describe('v4 fee, hooks, and ranking',()=>{
 ...
 it('ranks eligibility and reliable valuation before raw liquidity',()=>{
  const base={protocolVersion:'v4' as const,poolId:`0x${'2'.repeat(64)}`,key,target:{address:currency0,symbol:'AI',decimals:18},funding:{address:currency1,symbol:'USDG',decimals:6},targetIndex:0 as const,fundingIndex:1 as const,priceFundingPerTarget:1,priceProvenance:'test',feeLabel:'1%',tickSpacing:200,trustedTvlUsd:null,feeSemantics:decodeV4Fee(10_000,10_000),hookStatus:classifyV4Hooks(zeroAddress),refreshBlock:1n,blockers:[],initializationBlock:1n} satisfies Omit<V4Candidate,'liquidity'|'valuation'|'executionEligible'>;
 ...
 it('ranks fresh trusted TVL within the same eligibility class without hiding unknown or unsupported pools',()=>{
  const base={protocolVersion:'v4' as const,key,target:{address:currency0,symbol:'AI',decimals:18},funding:{address:currency1,symbol:'USDG',decimals:6},targetIndex:0 as const,fundingIndex:1 as const,priceFundingPerTarget:1,priceProvenance:'test',liquidity:1n,feeLabel:'1%',tickSpacing:200,feeSemantics:decodeV4Fee(10_000,10_000),hookStatus:classifyV4Hooks(zeroAddress),valuation:{status:'unavailable' as const,reason:'not used for ranking',provenance:'test'},refreshBlock:1n,blockers:[],initializationBlock:1n};
  const row=(id:string,tvl:number|null,executionEligible=true)=>({...base,poolId:`0x${id.repeat(64)}`,trustedTvlUsd:tvl,executionEligible});
  const ranked=rankV4Candidates([row('d',null),row('c',null),row('b',30_000),row('a',500_000),row('e',2_000_000,false)]);
 expect(ranked.map(x=>x.poolId.slice(2,3))).toEqual(['a','b','c','d','e']);
  expect(ranked).toHaveLength(5);
 });
 it('treats stale $10M as unknown behind fresh $50K',()=>{
  const now=1_700_000_000_000,stale=trustedV4PoolUsdMetric({tvl_usd:10_000_000,tvl_source:'test',tvl_observed_at_ms:now-60_000,tvl_fresh_until_ms:now-1,tvl_status:'fresh'},now).usd,fresh=trustedV4PoolUsdMetric({tvl_usd:50_000,tvl_source:'test',tvl_observed_at_ms:now-1_000,tvl_fresh_until_ms:now+60_000,tvl_status:'fresh'},now).usd;
  const base={protocolVersion:'v4' as const,key,target:{address:currency0,symbol:'AI',decimals:18},funding:{address:currency1,symbol:'USDG',decimals:6},targetIndex:0 as const,fundingIndex:1 as const,priceFundingPerTarget:1,priceProvenance:'test',liquidity:1n,feeLabel:'1%',tickSpacing:200,feeSemantics:decodeV4Fee(10_000,10_000),hookStatus:classifyV4Hooks(zeroAddress),valuation:{status:'unavailable' as const,reason:'not used',provenance:'test'},refreshBlock:1n,executionEligible:true,blockers:[],initializationBlock:1n};
  expect(rankV4Candidates([{...base,poolId:`0x${'a'.repeat(64)}`,trustedTvlUsd:stale},{...base,poolId:`0x${'b'.repeat(64)}`,trustedTvlUsd:fresh}]).map(x=>x.poolId.slice(2,3))).toEqual(['b','a']);
 });
});
