...
import { FallbackRpc, robinhoodMainnet, type RpcFailoverEvent } from '@funi/core';
import { auditRobinhoodV4Deployments, inspectV4Pool, V4_ROBINHOOD_DEPLOYMENTS, type V4PoolKey } from '@funi/v4';
...
describe('v4 StateView failover',()=>{
 ...
 it('keeps every final-preflight read on FallbackRpc and the broadcast client separate',()=>{
  const adapter=readFileSync('packages/uniswap-v4-adapter/src/index.ts','utf8'),transaction=readFileSync('apps/cli/src/transaction-boundary.ts','utf8');
